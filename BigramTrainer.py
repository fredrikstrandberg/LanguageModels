...
class BigramTrainer(object):
    ...
    def process_token(self, token):
        ...
        if self.unigram_count[token] == 1:  # om ordet är nytt
    ...
    def stats(self):
        ...
        bigram_print = []
        for object in self.unigram_count:
            rows_to_print.append(str(self.index[object]) + " " + object + " " + str(self.unigram_count[object]))
            for word in self.bigram_count[object]:
                bigram_print.append(str(self.index[object]) + " " + str(self.index[word]) + " " +
                                    str('%.15f' % (math.log(self.bigram_count[object][word]/self.unigram_count[object]))))

        for row in bigram_print:
